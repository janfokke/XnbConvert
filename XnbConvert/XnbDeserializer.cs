...
namespace XnbConvert
{
    public class XnbDeserializer
    {
        ...
        public XnbDeserializer(XnbConvertSettings xnbConvertSettings)
        {
        ...
        public XnbFile<T> Deserialize<T>(Stream stream)
        {
            ...
            var xnbReaderManager = new XnbReaderManager(XnbConvertSettings.XnbReaderTypeResolver);
            ...
            var readerIndex = xnbStreamReader.Read7BitEncodedInt();
            var reader = xnbReaderManager.XnbReaderFromIndex(readerIndex);
            if (reader == null)
            {
                xnbResult.Content = default;
            }
            else
            {
                var read = reader.Read(xnbReaderManager, xnbStreamReader);
                if (read is T content)
        ...
        private XnbStreamReader DecodeXnbStreamReader(XnbStreamReader endcodedXnbStreamReader, XnbFlags flags, int size)
        {
            ...
            var decompressedStream =
                new LzxDecoderStream(endcodedXnbStreamReader.BaseStream, decompressedSize, compressedSize);
