...
{
    [TestFixture]
    public class XnbNullableTypeReaderTests
    {
        [Test]
        ...
        {
            int?[] nullableInt = { 1 ,2 ,3};
            byte[] rawData = XnbConvert.Serialize(nullableInt);
            File.WriteAllBytes("testy2.xnb", rawData);
            XnbFile<int?[]> val = XnbConvert.Deserialize<int?[]>(rawData);
            Assert.AreEqual(nullableInt, val.Content);
        }
    }
}